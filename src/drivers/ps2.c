...
process_scan(unsigned scode)
{
	...
	else if ( HOME <= ch && ch <= INSRT )
	{
		...
		(getDriver(INPUT_DRIVER)->ioctl_driver)(INPUT_SETFOCUS,1,ch - AF1 + 1);		// AltFn: cambio de foco
	else
	{
...
int
mt_ps2_getlayout(const char ** name)
{
	if (name == NULL)
...
int
mt_ps2_setlayout(const char *name, bool* valid)
{
	if (name==NULL)
		return ERR_INVALID_ARGUMENT;
	unsigned n;
...
int
mt_ps2_layouts(const char*** kbdnames)
{
	if (kbdnames==NULL)
		return ERR_INVALID_ARGUMENT;
	*kbdnames = names; 
...
int read_driver_ps2(char *buf, unsigned size){
	return ERR_NO_METHOD_EXIST;
}

int write_driver_ps2(char *buf, unsigned size){
	return ERR_NO_METHOD_EXIST;
}
	...
	switch(type){
		...
		case PS2_SETLAYOUT: {
			if (minor!=2)
				...
				char* name = va_arg(pa, char*);
				bool* valid = va_arg(pa, bool*);
				rta = mt_ps2_setlayout(name,valid);
			}
			break;
		}
		case PS2_LAYOUTS: {
			if (minor!=1)
				...
				const char*** knames = va_arg(pa, const char***);
				rta = mt_ps2_layouts(knames);
			}
			break;
		}
		default:
